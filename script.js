...
function updateBtcMovement(id, updatedData) {
    ...
    // Ajustar el saldo de BTC según el cambio
    ...
    updateBTCPrice();
    renderBTCHistory();
    refreshAllViews(); // para actualizar dashboard y otros
        ...
        <div class="transaction-item">
            <div class="transaction-info">
                <div class="transaction-icon ${h.type === 'compra' ? 'ingreso' : 'gasto'}">
                    ${h.imageUrl ? `<img src="${h.imageUrl}" style="width:40px;height:40px;object-fit:cover;border-radius:12px;">` : `<i class="fas ${h.type === 'compra' ? 'fa-arrow-down' : 'fa-arrow-up'}"></i>`}
                </div>
                <div class="transaction-details">
                    <div class="transaction-description">${h.type === 'compra' ? 'Compra de BTC' : 'Venta de BTC'}${h.note ? `: ${escapeHtml(h.note)}` : ''}</div>
            ...
            <div class="transaction-actions">
                <button class="btn-edit" onclick="editBtcMovement('${h.id}')"><i class="fas fa-pencil-alt"></i></button>
                <button class="btn-delete" onclick="deleteBtcMovementHandler('${h.id}')"><i class="fas fa-trash-alt"></i></button>
            </div>
        </div>
    `).join('');
}
...
function refreshAllViews() {
    ...
    renderExpenseReport();
    renderBudgetPieChart(); // nuevo gráfico de presupuestos
}
...
function renderDashboard() {
    ...
    updateExpenseChart();
    updateIncomeChart();  // nuevo gráfico de ingresos
    updateBalanceChart();
}
...
// ========== RENDERIZADO DE LISTAS (continuación) ==========
...
function renderBudgetsList() {
    ...
    // Mostrar total presupuestado y gráfico de torta
...
function renderBudgetPieChart() {
    ...
    monthly.forEach(b => {
        const cat = categories.find(c => c.id === b.categoryId);
        labels.push(cat ? cat.name : 'Sin categoría');
        data.push(b.amount);
    });
    const colors = labels.map((_, i) => `hsl(${(i * 360 / Math.max(labels.length,1)) % 360}, 70%, 60%)`);
...
// ========== REPORTE DE GASTOS con ordenamiento por porcentaje ==========
...
function renderExpenseReport() {
    ...
    if (reportType === 'category') {
        ...
        currentExpensePercentData = labels.map((label, idx) => ({
            category: label,
            amount: data[idx],
            percentage: total > 0 ? (data[idx] / total) * 100 : 0
        }));
        ...
        let html = '<table class="transactions-table"><thead><tr><th>Categoría</th><th>Monto</th><th>Porcentaje</th></tr></thead><tbody>';
        ...
        html += `<tr style="font-weight:bold;"><td>Total</td><td>${formatCurrency(total)}</td><td>100%</td></tr></tbody></table>`;
        document.getElementById('expenseReportDetails').innerHTML = html;
        
        // Actualizar tabla de porcentajes con ordenamiento
        ...
        if (percentBody) percentBody.innerHTML = '<tr><td colspan="3">Selecciona "Por Categoría" para ver porcentajes</td></tr>';
    }
